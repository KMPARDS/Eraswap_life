      ...
      fallback: [
        {
          ...
          functions: {
            ...
            utxos: {
              url: (address) =>
                `https://api.blockcypher.com/v1/btc/main/addrs/${address}/full?token=c29426c605e541bea307de3a54d94fcf&unspentOnly=true&includeHex=true`,
                ...
                for (const tx of apiOutput.data.txs) {
                  ...
                  if (tx.next_outputs) {
                    ...
                    do {
                      longTx = (
                        await axios.get(
                          `https://api.blockcypher.com/v1/btc/main/txs/${tx.hash}?limit=${limit}`
                        )
                      ).data;
                      console.log({ longTx });
                      limit *= 10;
            ...
            txPush: {
              url: () =>
                'https://api.blockcypher.com/v1/btc/main/txs/push?token=c29426c605e541bea307de3a54d94fcf',
              body: (hex) => {
                tx: hex;
              },
              parse: (apiOutput) => {
      ...
      fallback: [
        {
          ...
          functions: {
            ...
            utxos: {
              url: (address) =>
                `https://api.blockcypher.com/v1/btc/test3/addrs/${address}/full?token=c29426c605e541bea307de3a54d94fcf&unspentOnly=true&includeHex=true`,
              parse: async (apiOutput, userAddress) => {
                const address = apiOutput.data.address || userAddress;
                const unspent = [];
                for (const tx of apiOutput.data.txs) {
                  ...
                  if (tx.next_outputs) {
                    ...
                    do {
                      longTx = (
                        await axios.get(
                          `https://api.blockcypher.com/v1/btc/test3/txs/${tx.hash}?limit=${limit}`
                        )
                      ).data;
                      console.log({ longTx });
                      limit *= 10;
            ...
            txPush: {
              url: () =>
                'https://api.blockcypher.com/v1/btc/test3/txs/push?token=c29426c605e541bea307de3a54d94fcf',
              body: (hex) => {
                tx: hex;
              },
              parse: (apiOutput) => {
      ...
      fallback: [
        {
          ...
          functions: {
            balance: {
              getBalanceUrl: (address) =>
                `https://bch-chain.api.btc.com/v3/address/${address}`,
              parseBalanceFromApiOutput: (apiOutput) => {
                if (!apiOutput) return null;
